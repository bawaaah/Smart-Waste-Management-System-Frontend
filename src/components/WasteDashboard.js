...
const WasteDashboard = () => {
    const { deviceId, deviceType, userId } = useParams(); // Get deviceId from URL parameters
    ...
    useEffect(() => {
        const fetchWasteData = async () => {
            try {
                const response = await axios.get(`http://localhost:3000/api/waste/device/${deviceId}`);
    ...
    return (
        <div className="container mx-auto p-4">
            ...
            {showForm && <AddWasteRecordForm deviceId={deviceId} deviceType={deviceType} userId={userId}/>} {/* Pass deviceId to the form */}
